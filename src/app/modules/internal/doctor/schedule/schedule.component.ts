...
import { DoctorScheduleByDay } from '../../../core/models/schedule.model';
import { MatSnackBar } from '@angular/material/snack-bar';

@Component({
  selector: 'app-schedule',
  standalone: true,
  imports: [MatIcon, MatIconButton, NgForOf, NgIf, MatButton],
...
export class ScheduleComponent implements OnInit {
  ...
  createDailySchedule() {
    void this._router.navigate([`/internal/my-schedules/daily`], {
      state: { scheduleId: 1 },
    });
  }

  createWeeklySchedule() {
    void this._router.navigate([`/internal/my-schedules/weekly`], {
      state: { scheduleId: 2 },
    });
  }

  goToDetails(scheduleId: number) {
    void this._router.navigate([`/internal/my-schedules/details`], {
  ...
  private loadSlots(): void {
    ...
    this._scheduleService
      .getSchedules(this.currentWeekStart, this.currentWeekEnd, 0, 50)
      .subscribe({
        next: (allSchedules) => {
          const mapped: DoctorScheduleByDay = {};
