...
import { DoctorCreateAbsence } from '../../../../core/models/absences.model';
...
export class CreateAbsenceDialogComponent {
  ...
  createAbsence(): void {
    ...
    const absenceData: DoctorCreateAbsence = {
      name: formValue.name,
      startedAt: toLocalISOString(formValue.startedAt!),
      finishedAt: toLocalISOString(endOfDay),
    };

    this._absenceService.createAbsence(absenceData).subscribe({
      next: () => {
